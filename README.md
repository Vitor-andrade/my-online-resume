...
- A single-page landing: Hero / Summary, Skills, Work Experience, Education, Key Achievements, Projects, Certifications, Contact.
- A **filterable Skills explorer** — categories narrow with a pill bar, and each technology shows its brand icon, led by a featured **AI & LLM Engineering** group.
- A **Work Experience timeline** with pulsing chronological nodes.
- **Illustrated Key Achievement case studies** — each `/achievements/[slug]` route opens a cover image, a per-chapter narrative (highlights and impact) and a screenshot gallery; multi-part work (web + mobile) is told in chapters.
- **Projects** sourced from the owner's pinned GitHub repositories, alongside curated entries.
- **Linked credentials** — certifications, diplomas and the IELTS result open their certificate in a new tab.
...
- **Light / dark theme** built on a token-driven design system, with tasteful scroll-reveal motion.
...
| **Styling**       | Tailwind CSS v4                                                     | Fast, consistent, utility-driven styling. Light/dark theming is a small in-house solution — a pre-paint script plus the `dark` class — with no flash of the wrong theme and no extra dependency.                                                                                                      |
...
| **Animation**     | Framer Motion (`motion`) + CSS keyframes                            | Tasteful, restrained motion — scroll-reveals, hover lifts, a pulsing experience timeline and a scroll hint — always honoring `prefers-reduced-motion`.                                                                                                                                                |
...
| **Analytics**     | Vercel Analytics + PostHog                                          | Core Web Vitals and product analytics, both on free tiers. Dedicated error tracking (Sentry) is a deferred TODO.                                                                                                                                                                                      |
